...
export default function FakAlshafraHome() {
  ...
  return (
    <div className="w-full min-h-screen bg-[#020617] flex flex-col items-center justify-center py-10 px-4 font-sans text-right" dir="rtl">
      ...
      {/* المربع الرئيسي (الجدول) */}
      <div className="w-full max-w-2xl bg-slate-900/50 border border-slate-800/60 rounded-[2rem] p-6 sm:p-8 shadow-xl flex flex-col gap-4 relative">
        
        {/* زر العودة: فوق اسم اللعبة، صغير جداً، ومجرد إيموجي */}
        <div className="flex justify-start">
          <button 
            onClick={() => router.push("/")} 
            className="text-lg hover:scale-110 transition-transform p-1 opacity-70 hover:opacity-100"
        ...
        <section className="space-y-4">
          <div className="bg-slate-950/50 border border-slate-800/50 p-5 rounded-2xl space-y-3">
            <h3 className="text-teal-400 font-black text-sm border-b border-slate-800 pb-2 mb-3">طريقة اللعبة 📜</h3>
            <ul className="text-slate-300 text-xs sm:text-sm font-bold space-y-2.5 leading-relaxed list-disc list-inside px-2">
              ...
              <li>المفككون يتشاورون ويرشحون الكلمات التابعة لفريقهم بناءً على الشفره ويجب ان يصلوا لجميع كلمات فريقهم قبل الفريق المنافس</li>
